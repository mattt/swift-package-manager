...
extension Workspace {
    ...
    func download(
        ...
        registryManager.downloadSourceArchive(for: version, of: package, into: fileSystem, at: path, expectedChecksum: nil /* FIXME */, on: queue) { result in
    ...
    private func fetch(
        ...
        case .version(let version):
            ...
            // this?
            // FIXME: this should not block
            let container = try temp_await { containerProvider.getContainer(for: package, skipUpdate: true, on: self.queue, completion: $0) }
