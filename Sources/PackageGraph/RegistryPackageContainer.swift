...
public final class RegistryPackageContainer {
    ...
    static func create(
        ...
        let container = RegistryPackageContainer(package: package,
                                                 mirrors: mirrors,
                                                 registryManager: registryManager,
        ...
        registryManager.fetchVersions(of: package, on: queue) { result in
            switch result {
    ...
    private func manifest(for version: Version) throws -> Manifest {
        try manifestsCache.memoize(version) {
            try tsc_await { registryManager.fetchManifest(for: version, of: package, using: manifestLoader, on: .global(), completion: $0) }
        }
    }
}
...
extension RegistryPackageContainer: PackageContainer {
    ...
    public func getUpdatedIdentifier(at boundVersion: BoundVersion) throws -> PackageReference {
        ...
        return try package.with(newName: self.manifest(for: version).name)
    }
}
